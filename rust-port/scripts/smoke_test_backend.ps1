...
function Assert-SuccessOrAcceptedState {
    param(
        [Parameter(Mandatory = $true)]$Response,
        [Parameter(Mandatory = $true)][string]$FailureLabel,
        [string[]]$AcceptedMessagePatterns = @()
    )
    ...
    foreach ($pattern in $AcceptedMessagePatterns) {
        if (($Response.message -as [string]) -match $pattern) {
            Write-Host ("{0}: accepted existing staging state: {1}" -f $FailureLabel, $Response.message)
            return
        }
    }

    throw "${FailureLabel}: $($Response.message)"
}
...
Assert-Flag -Condition ($readiness.status -in @('ok', 'ready')) -Message "Readiness endpoint did not report ready: $($readiness.status)"
foreach ($check in $readiness.checks) {
    Assert-Flag -Condition ($check.status -eq 'ok') -Message "Readiness check '$($check.key)' reported '$($check.status)': $($check.detail)"
}
...
$executionAlreadyTerminal = $executionScreen.status_label -in @('Delivered', 'Paid Out', 'Settled', 'Released')
if ($executionAlreadyTerminal) {
    Write-Host ("Execution lifecycle already complete for seeded leg {0}; preserving staging state." -f $BookingLegId)
} else {
...
if ($executionAlreadyTerminal) {
    Write-Host ("Payments lifecycle already complete for seeded leg {0}; preserving staging state." -f $BookingLegId)
} else {
...
$connectLink = Assert-Envelope -Response (Invoke-StloadsApi -Method Post -Path '/payments/connect/onboarding-link' -BearerToken $carrierToken -Body @{
    return_url = 'https://portal.stloads.com/settings/payouts?done=1'
    refresh_url = 'https://portal.stloads.com/settings/payouts?refresh=1'
}) -Context 'Carrier Connect onboarding link'
Assert-Flag -Condition ($connectLink.success) -Message "Connect onboarding link failed: $($connectLink.message)"
Assert-Flag -Condition (-not [string]::IsNullOrWhiteSpace($connectLink.account_id)) -Message 'Connect onboarding response did not include a Stripe account id.'
...
$stripeWebhook = Assert-Envelope -Response (Invoke-StloadsApi -Method Post -Path '/payments/webhooks/stripe' -BearerToken $stripeWebhookToken -ExtraHeaders $stripeWebhookHeaders -Body @{
    ...
    stripe_account_id = $connectLink.account_id
...
Assert-SuccessOrAcceptedState -Response $offerReview -FailureLabel 'Offer review failed' -AcceptedMessagePatterns @('already', 'accepted', 'booked', 'not pending')
...
Assert-SuccessOrAcceptedState -Response $resolveSyncError -FailureLabel 'Sync error resolution failed' -AcceptedMessagePatterns @('already', 'resolved', 'not found', 'missing')
...
Assert-SuccessOrAcceptedState -Response $close -FailureLabel 'TMS close failed' -AcceptedMessagePatterns @('already', 'closed', 'withdrawn', 'terminal')
