...
use shared::{
    AuthOnboardingScreen, ForgotPasswordRequest, LoginRequest, MfaVerifyRequest, OtpPurpose,
    RegisterRequest, ResetPasswordRequest, SubmitOnboardingRequest, VerifyOtpRequest,
};
    ...
    Effect::new(move |_| {
        query.with(|params| {
            ...
            if challenge_id.get_untracked().is_empty() {
                if let Some(value) = params.get("challenge") {
...
fn build_mfa_path(email: &str, challenge_id: &str) -> String {
    format!(
