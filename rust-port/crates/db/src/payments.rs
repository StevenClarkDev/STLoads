...
pub async fn record_stripe_webhook_once(
    ...
    ensure_stripe_webhook_event_store(pool).await?;
...
async fn ensure_stripe_webhook_event_store(pool: &DbPool) -> Result<(), sqlx::Error> {
    sqlx::query(
    ...
    sqlx::query(
        "CREATE UNIQUE INDEX IF NOT EXISTS uq_stripe_webhook_events_event_id
         ON stripe_webhook_events (tenant_id, event_id)",
    )
    .execute(pool)
    .await?;
    sqlx::query(
        "CREATE INDEX IF NOT EXISTS idx_stripe_webhook_events_payment_intent
         ON stripe_webhook_events (tenant_id, payment_intent_id, event_type)",
    )
    .execute(pool)
    .await?;
    sqlx::query(
        "CREATE INDEX IF NOT EXISTS idx_stripe_webhook_events_leg
         ON stripe_webhook_events (tenant_id, leg_id, processed_at DESC)",
    )
    .execute(pool)
    .await?;
    Ok(())
