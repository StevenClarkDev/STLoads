@extends('admin-layout.app')
@section('content')
   <div class="col-xl-12 box-col-6 p-3">
    <div class="row">
        <div class="col-md-9">
            <div class="card">
                <div class="card-body">
                    <h5 class="mb-3">Equipments Edit</h5>
                    <form class="card-body" method="POST" action="{{ route('equipments.update', $equipment->id) }}">
                        @csrf
                        @method('PUT')
                        <div class="row g-4">
                            <div class="col-md-6">
                                <div class="form-floating form-floating-outline">
                                    <input type="text" id="multicol-username" class="form-control" placeholder="Enter Name"
                                        name="name" value="{{ $equipment->name }}" />
                                    <label for="multicol-username">Name</label>
                                </div>
                            </div>
                        </div>
                        <div class="d-flex flex-row-reverse gap-1 mt-2">
                            <button type="submit" class="btn btn-outline-primary">Submit</button>
                            <a href="{{ route('equipments.index') }}" type="back" class="btn btn-outline-secondary">Back</a>
                        </div>
                    </form>
                </div>
            </div>
        </div>
    </div> <!-- End of .row -->
</div>
@endsection
